...
## LA CURVATURA DEL MUNDO (el "rolling log"): PROBADA Y RETIRADA (1-9-2026)

Se montó el efecto de Animal Crossing sobre el mapa —la geometría hundiéndose
según se aleja del punto que enfoca la cámara— y **el usuario lo retiró**: la
idea le gustaba, pero destartalaba el mapa (sombras, alturas, cámara y
descuadres) y cuadrarlo pedía demasiados intentos. **No reintroducirlo sin que
lo pida.** Se recupera entero con:

    git checkout curvatura-rolling-log -- shaders/curvatura.gdshader

Lo que se aprendió, por si algún día se retoma:
- **Se curva el EJE DE SCROLL, no la profundidad de cámara**: esta cámara es
  ortogonal y el mapa una cinta vertical. Y el pivote va en el punto ENFOCADO,
  no en la cámara —que está 30 u por detrás y mete todo en el tramo empinado de
  la parábola, cizallando los modelos.
- **Hay que SUSTITUIR el material**: `StandardMaterial3D` no admite código de
  vértice y `next_pass` es una pasada MÁS, no una modificación de la primera.
  Ahí empieza media lista de descuadres: cada material sustituido pierde lo que
  no se le copie a mano.
- **Y hay que repetir la cuenta en GDScript**, porque los botones 2D se colocan
  proyectando el punto 3D y el hundimiento ocurre en el SHADER.
- **Aquí NUNCA se ve el horizonte** (la cámara mira el agua desde arriba), así
  que no daba la silueta curva de Animal Crossing: solo que lo lejano se
  hundiera. **Y NO ahorraba recursos**: coste medido 0,661 / 0,669 / 0,683 ms
  por fotograma a fuerza 0,016 / 0,009 / 0,004, o sea dentro del ruido, pero
  ahorro directo ninguno.
