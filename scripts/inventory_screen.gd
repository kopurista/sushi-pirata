...
func _open_recipe_sheet(id: String) -> void:
	...
	if not known:
		...
		if not mejora.is_empty():
			body.add_child(_section_title("Versión mejorada"))
			body.add_child(_build_upgrade_block(id, mejora))
...
func _build_clients_block(data: Dictionary) -> Control:
	...
	for t in CLIENT_TYPES:
		...
		l.text = "%s: %s · %d%%" % [CLIENT_NAMES[t], _chance_text(chance),
...
func _build_upgrade_block(base_id: String, mejora: Dictionary) -> Control:
	...
	col.add_child(como)
	col.add_child(_build_clients_block(up))
	return col
