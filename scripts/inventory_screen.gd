	...
	for id in RecipeData.RECIPES:
		...
		if filter_client != "":
			...
			if only_f != "" and only_f != filter_client:
				continue
			if RecipeSheet.forced_chance(data, filter_client, tier) < 0.4:
				continue
		out.append(id)
...
func _open_recipe_sheet(id: String) -> void:
	# LA FICHA VIVE EN `RecipeSheet`, no aqui: la abre tambien el SELECTOR de
	# recetas, y dos copias del mismo panel acabarian contando cosas
	# distintas del mismo plato.
	RecipeSheet.abrir(ui, id)
