...
def build_pot_carta() -> None:
    """LA CARTA del cartel de potenciadores (`pot_carta.png`).

    GENERADA CON LUDO (`_gen/ui2/pot/carta.webp`), no dibujada: la primera
    version salia de `ImageDraw` —pergamino con marco de oro— y el usuario la
    rechazo, que es la misma leccion de la perilla de los deslizadores: una
    forma geometrica hecha a mano se ve plana al lado del resto del set.
    Esta es madera tallada con herrajes de laton en las esquinas y su hoja de
    pergamino dentro.

    **NO ES UN 9-SLICE**: los herrajes SOBRESALEN del marco recto (medido: en
    mitad de cada canto el alfa es 0, porque la caja del recorte la marcan las
    esquinas), asi que estirando las bandas del medio saldrian franjas
    transparentes por los lados. Se usa ENTERA y a su proporcion exacta
    (0.690), y por eso la carta del juego mide 192x278: ver
    `level3d._card_vertical`, que la dibuja con STRETCH_SCALE.

    Se exporta a 300 de ancho —mas de los 192 a los que se dibuja— porque el
    movil renderiza a mas resolucion que el lienzo de diseno.
    """
    img = crop_alpha(Image.open(RAW / "pot" / "carta.webp").convert("RGBA"))
    save(fit_width(img, 300), "pot_carta")
